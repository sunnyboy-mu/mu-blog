...
> 具体代码，请移步[defer 优化白屏](/views/front-end/vue3-hooks.html#_2、-defer优化白屏)
...
```vue
<template>
  <div :id="id" class="marquee-wrapper">
    <span :style="translate" class="marquee-inside">
      <slot></slot>
    </span>
    <span :class="`marquee-fake ${id}-fake`" :style="fakeTranslate"></span>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    createFakeDom(inside) {
      ...
      if (this.type === "horizontal") {
        fakeDom.style.marginLeft = this.margin + "px";
      } else {
        fakeDom.style.marginTop = this.margin + "px";
      }
    },
    run(insideOffset) {
      this.distance -= this.speed;
      if (this.distance === -insideOffset - this.margin)
        this.distance = insideOffset + this.margin;
      this.fakeDistance -= this.speed;
      if (this.fakeDistance === -insideOffset * 2 - this.margin)
        this.fakeDistance = this.margin;
    },
  },
};
</script>
